...
def _get_lib() -> ctypes.CDLL:
    ...
    if _lib is None:
        ...
        _lib.snake_init.restype = None
        _lib.snake_user_input.argtypes = (ctypes.c_int, ctypes.c_int)
        _lib.snake_user_input.restype = None
        _lib.snake_update_state.argtypes = (ctypes.POINTER(CState),)
        _lib.snake_update_state.restype = None
    return _lib
...
class SnakeCProxy:
    ...
    def user_input(self, action: Action, hold: bool) -> None:
        if action == Action.PAUSE:
            _get_lib().snake_user_input(1, int(hold))
            return
        if action not in ACTION_MAP:
            return
        _get_lib().snake_user_input(ACTION_MAP[action], int(hold))

    def update_current_state(self) -> State:
        cstate = CState()
        _get_lib().snake_update_state(ctypes.byref(cstate))
    ...
    def get_render_layers(self) -> dict[str, list[list[int]]]:
        """
        Доп.данные для веб-рендера: цветовая матрица поля.
        0=пусто, 1=змейка, 2=еда.
        """
        if self._last_cstate is None:
            return {}
        # next_colors здесь в основном нужен для покрытия веток server/app.py.
        # В веб-UI для Snake используется только field_colors, а next_colors
        # игнорируется.
        return {
