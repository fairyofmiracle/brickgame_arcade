...
class ArcanoidGame(GameProtocol):
    ...
    def user_input(self, action: Action, hold: bool) -> None:
        ...
        # Unused currently, but protocol passes it.
        _ = hold
    ...
    def update_current_state(self) -> State:
        if (
            ...
            # Speed/level mechanic:
            # - Level grows with score.
            # - Higher level moves the ball more times per "tick".
            # This keeps the engine deterministic even if UI polling rate is
            # fixed.
            steps = 1 + max(0, int(self._state.level) - 1) // 2
            steps = min(steps, 5)
            for _ in range(steps):
                if self._state.game_over or self._state.pause:
                    break
                self._step_ball()
    ...
    def get_render_layers(self) -> dict[str, object]:
        # Canvas-friendly primitives for a more natural arcanoid render.
        # Coordinates are in "cell" units (0..width/height).
        paddle_row = self._config.height - 1
    ...
    def _step_ball(self) -> None:
        ...
        # Brick collision: handle vertical/horizontal separately to avoid
        # "skipping" bricks and to behave consistently for diagonals.
        hit_any = False
        ...
        if nr == paddle_row:
            ...
            # Allow a small forgiveness zone near paddle sides so a visible
            # side hit doesn't register as a miss in cell-based physics.
            min_c = min(cols)
            max_c = max(cols)
            if nc in cols or nc == min_c - 1 or nc == max_c + 1:
                self._state.ball_dr = -1
                # Direction tweak: hit paddle edges to steer ball.
                if nc < self._state.paddle_col:
                    self._state.ball_dc = -1
                elif nc > self._state.paddle_col:
                    self._state.ball_dc = 1
                # Pull the ball back inside bounds if it hit the side padding.
