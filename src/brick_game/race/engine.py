...
from brick_game.race.models import (
    Col, Pos, RaceConfig, RaceState, RaceStatus,
)
...
def _level_from_score(score: int) -> int:
    return min(
        RACE_MAX_LEVEL,
        1 + max(0, int(score)) // RACE_POINTS_PER_LEVEL,
    )
...
def _load_high_score() -> int:
    try:
        with open(
            _RACE_HIGH_SCORE_FILE, "r", encoding="utf-8"
        ) as f:
            raw = f.read().strip()
...
def _store_high_score(value: int) -> None:
    try:
        ensure_scores_dir()
        with open(
            _RACE_HIGH_SCORE_FILE, "w", encoding="utf-8"
        ) as f:
...
class RaceGame(GameProtocol):
    ...
    def __init__(
        self, config: RaceConfig | None = None
    ) -> None:
        ...
        self._lane_centers: List[int] = [
            1, 5, 8
        ]
        self._state.player_col = (
            self._lane_centers[1]
        )

    def user_input(
        self, action: Action, hold: bool
    ) -> None:
        if action == Action.START:
            hs = self._state.high_score
            self._state = RaceState(
                status=RaceStatus.RUNNING, high_score=hs
            )
        ...
        if (
            action == Action.PAUSE
            and self._state.status == RaceStatus.RUNNING
        ):
        ...
        if (
            action == Action.PAUSE
            and self._state.status == RaceStatus.PAUSED
        ):
    ...
    def update_current_state(self) -> State:
        ...
        draw(
            pr, self._state.player_col, player_sprite
        )
    ...
    def _step(self) -> None:
        ...
        level_step = _enemy_step_for_level(
            self._state.level
        )
        step = level_step * (
            2 if self._accelerated else 1
        )
        ...
        if passed:
            ...
            self._state.level = _level_from_score(
                self._state.score
            )
            if (
                self._state.score
                > self._state.high_score
            ):
                self._state.high_score = (
                    self._state.score
                )
    ...
    def _spawn_interval_ticks(self) -> int:
        ...
        if self._accelerated:
            return max(
                _NITRO_SPAWN_MIN_TICKS,
                base // _NITRO_SPAWN_DIVISOR,
            )
        return base

    def _spawn_enemy(
        self, convoy: bool = False
    ) -> None:
        top = _SPRITE_HEIGHT + _MIN_VERTICAL_GAP
        if any(
            r <= top
            for r, _ in self._state.enemies
        ):
            return

        col: Col = random.choice(
            self._lane_centers
        )
